...
**Status**: `0.2.1` alpha. The current MVP ships seven guard scanners:
...
jobs:
  agent-guard:
    ...
    steps:
      ...
      - uses: yui-stingray/agent-guard@v0.2.1
        with:
...
```json
{
  ...
  "tool": {"name": "agent-guard", "version": "0.2.1"},
  "scanner": "context",
  "status": "ok",
  "exit_code": 0,
...
PY
gh attestation verify dist-verify/yui_agent_guard-0.2.1-py3-none-any.whl \
...
gh attestation verify dist-verify/yui_agent_guard-0.2.1.tar.gz \
  --repo yui-stingray/agent-guard \
  --signer-workflow yui-stingray/agent-guard/.github/workflows/release.yml \
  --source-ref refs/tags/v0.2.1
```
