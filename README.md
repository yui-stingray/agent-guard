...
**Source status**: `0.3.5.dev0` alpha (unreleased). The latest published release
is `0.3.4`. Vendor-neutral, static-only, Python 3.11.4+, with one runtime
dependency (`PyYAML`).

> [!WARNING]
> **Published 0.3.4 known risk.** In `0.3.4`, an unreviewed repository-controlled
> custom context-policy regular expression can consume excessive CPU during
> `context check` or `report`. Do not run those commands with `0.3.4` against a
> policy changed by an untrusted contribution; use a maintainer-reviewed policy
> from a trusted revision or review and reject policy changes before execution.
> The copyable pull-request workflow below performs that policy-diff preflight
> before invoking the immutable Action and applies a one-minute step timeout.
> Master (`0.3.5.dev0`) isolates and bounds that matching, but it is unreleased:
> no fixed package or Action release is being claimed, and the documented
> demand-validation release freeze remains in effect.

**Version gate.** Bound audit-event report/manifest v2 and
`agent-guard.public_agent_policy_audit_event.v1` are also unreleased source
features. Copyable install and Action examples intentionally remain pinned to
the published `0.3.4` artifacts and do not make those v2 features available.
...
> [!WARNING]
> The workflow generated by published `0.3.4` predates the context-policy
> preflight and Action timeout shown in the complete workflow under
> [Adoption and CI reference](#adoption-and-ci-reference). Do not run or commit
> that generated `.github/workflows/agent-guard.yml` unchanged. Replace it with
> the complete mitigated workflow before enabling pull-request CI.
...
the published `0.3.4` workflow must be replaced as warned above.
...
final gate runs the starter-policy baseline generated by `init --write`:
