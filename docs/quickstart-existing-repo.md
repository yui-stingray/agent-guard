...
> **Version gate:** package install examples use `0.3.8`, including bound
...
```console
uvx --python 3.12 --from yui-agent-guard==0.3.8 agent-guard init --root . --print
```
...
python3 -m venv .venv && \
  . .venv/bin/activate && \
  python -m pip install yui-agent-guard==0.3.8
...
```powershell
py -3.12 -m venv .venv
.\.venv\Scripts\python.exe -m pip install yui-agent-guard==0.3.8
...
Keep the pull-request preflight before the published `0.3.7` Action and keep
