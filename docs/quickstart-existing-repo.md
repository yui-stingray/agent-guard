...
> **Version gate:** the install and Action examples use published `0.3.4`.
> Bound audit-event report/manifest v2 and the
> `agent-guard.public_agent_policy_audit_event.v1` profile below require
> unreleased `0.3.5.dev0` source and are not available from those public pins.
...
> preflight and Action timeout in the complete [Green CI Path](#2-green-ci-path)
> workflow below. Do not run or commit the generated
> `.github/workflows/agent-guard.yml` unchanged. Replace it with that complete
> mitigated workflow before enabling pull-request CI.
