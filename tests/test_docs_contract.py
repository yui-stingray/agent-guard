...
def test_readme_distinguishes_unreleased_source_from_published_release() -> None:
    ...
    assert f"**Source status**: `{pyproject_version()}` alpha (unreleased)." in readme
    assert f"latest published release\nis `{ACTION_RELEASE_VERSION}`" in readme
...
def test_published_init_workflow_warning_precedes_adoption_commands() -> None:
    readme = README.read_text(encoding="utf-8")
    quickstart = EXISTING_REPO_QUICKSTART.read_text(encoding="utf-8")
    readme_adoption = readme[readme.index("### Adopt after review") : readme.index("## Why")]
    quickstart_initial = quickstart[: quickstart.index("## 2. Green CI Path")]

    for section in (readme_adoption, quickstart_initial):
        normalized = " ".join(section.split())
        assert "workflow generated by published `0.3.4` predates" in normalized
        assert "Do not run or commit" in normalized
        assert ".github/workflows/agent-guard.yml" in normalized
        assert "complete" in normalized
        assert "mitigated workflow" in normalized
...
def test_public_docs_gate_known_release_risk_and_unreleased_v2_features() -> None:
    readme = README.read_text(encoding="utf-8")
    security = SECURITY_POLICY.read_text(encoding="utf-8")
    ...
    for docs in (readme, security):
        assert "Published 0.3.4" in docs or "published 0.3.4" in docs
        assert "unreviewed" in docs
        assert "context" in docs
        assert "regular expression" in docs or "regex" in docs
        assert "0.3.5.dev0" in docs
        assert "unreleased" in docs
        assert "release freeze" in docs
    for docs in (readme, evidence_contracts, compatibility):
        assert "Version gate" in docs or "source features" in docs
