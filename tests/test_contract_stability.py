...
def test_changelog_records_latest_release_entry() -> None:
    ...
    assert normalized_unreleased == " ".join(
        [
            "- Marked master package and generated-evidence identity as PEP 440 `0.3.5.dev0` while public install, Action, and provenance examples remain pinned to published `0.3.4`. Added an explicit published/source feature gate and a bounded known-risk notice for `0.3.4` context-policy regular expressions without claiming a fixed release or lifting the release freeze.",
            ...
            "- Named the unreleased bound-event grammar `agent-guard.public_agent_policy_audit_event.v1` as a guard-owned public-safe subset of the underlying `agent-policy` v1.1 event shape. Bound v2 producers, schemas, and consumers now share one non-whitespace printable-ASCII, public-safe artifact-path grammar that rejects traversal, colons, controlled secret-shaped values, and embedded raw 64-hex hashes while released v1 path behavior remains unchanged.",
