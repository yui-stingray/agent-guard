...
def test_changelog_records_latest_release_entry() -> None:
    ...
    assert normalized_unreleased == " ".join(
        [
            "- Made static MCP package pin inference inspect recognized package-manager operands and selectors instead of arbitrary command arguments. Full SemVer, exact `uvx` requirements, and package-attached full SHA-256 pins remain eligible; ranges, npm-style major/minor-only versions, short digests, option values, and ambiguous layouts fail closed as unpinned metadata.",
