...
def test_release_mode_rejects_current_development_version() -> None:
    ...
    assert "requires a final versioned heading for release 0.3.10.dev0" in result.stderr
...
def test_release_notes_include_the_executable_changes() -> None:
    ...
    assert "canonical unpadded base32" in notes
    assert "private-key-header" in notes
